#Callan Murphy
#15/05/2018
#This program allows the user to play a game of hangman against the computer
#The game operates by storing 2 primary variables, the word and a hidden version of the word (asterisks). 
# When a letter is guessed, the letter in the word is replaced with an asterisk and the asterisk in the 
# hidden word is replaced with the letter. Once the entire word is asterisks, 
# the program tells the user that they have won
...
def main(t):
  ...
  categoryWord = random.choice(categories[userCategory]) #a random word is selected from the category chosen by the user
  word = [] #a list is initialized for the word that will be guessed
  for i in range(len(categoryWord)):
    word.append(categoryWord[i]) #each letter is appended to the list so that they can be individually changed later (into asterisks)
  ...
  print ("")
  print ("Category:", categoryStrings[userCategory]) #notifies the user of the category they selected
